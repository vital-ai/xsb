...
**
** $Id: cvarstring_thread2.c,v 1.1 2007-06-23 21:27:23 tswift Exp $
** 
*/
...
void *command_rs_err(void * arg) {
  ...
  if ((rc = xsb_command_string(r_th,"test_r_err.")) == XSB_ERROR)
   printf("Planned command error r: %s/%s\n",xsb_get_error_type(r_th),xsb_get_error_message(r_th));
  else if ((rc = xsb_command_string(r_th,"test_r_fail.")) == XSB_FAILURE)
    printf("++r_err failed improperly\n");
 return NULL;
}
...
void *close_query_ps(void * arg) {
  ...
  rc = xsb_query_string_string(p_th,"p(X,Y,Z).",&p_return_string,"|");
  if (rc == XSB_ERROR) 
    printf("++Closed query Error p: %s/%s\n",xsb_get_error_type(p_th),xsb_get_error_message(p_th));
  printf("Closed return p %s\n",(p_return_string.string));
  rc = xsb_close_query(p_th);
  if (rc == XSB_FAILURE) 
    printf("++Failure on closed query (p)!\n");
...
void *query_ps_err(void * arg) {
 ...
 if (rc == XSB_ERROR) 
   printf("Planned query Error p: %s/%s\n",xsb_get_error_type(p_th),xsb_get_error_message(p_th));
 return NULL;
}
...
{ 
  ...
  if (xsb_init_string(argv[1])) {
    printf("%s initializing XSB: %s\n",xsb_get_error_type(xsb_get_main_thread()),
  ...
  if (xsb_command_string(main_th, "consult('edb.P').") == XSB_ERROR)
    printf("++Error consulting edb.P: %s/%s\n",xsb_get_error_type(main_th),
  ...
  */
  ...
  if (rstatus != 0) printf("R join returns status %d\n",rstatus);
  qstatus = pthread_join(qthread_id,&qreturn);
  if (qstatus != 0) printf("Q join returns status %d\n",qstatus);
...
void *ll_command_ps(void * arg) {
  ...
  if ((rc = xsb_command(p_th)) == XSB_ERROR) 
   printf("LowLevel command error p: %s/%s\n",xsb_get_error_type(p_th),xsb_get_error_message(p_th));
...
void *ll_command_rs(void * arg) {
  ...
  if (xsb_command(r_th) == XSB_ERROR) 
   printf("LowLevel command error r: %s/%s\n",xsb_get_error_type(r_th),xsb_get_error_message(r_th));
